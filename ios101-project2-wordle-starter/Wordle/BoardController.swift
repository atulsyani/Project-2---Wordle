...
class BoardController: NSObject,
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
    // ...
      guard let themeString = settings[kWordThemeKey] as? String,
            let theme = WordTheme(rawValue: themeString) else {
          print("⚠️ Missing or invalid \(kWordThemeKey) in settings.")
          return
      }

      // 3) Generate a new goal word for that theme and assign it
      let newGoal = WordGenerator.generateGoalWord(with: theme)
      self.goalWord = newGoal

      // Optional: quick debug to confirm the change
      // print("✅ Theme set to \(theme.rawValue). New goalWord: \(goalWord)")
    // END YOUR CODE HERE
  }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    ...
    // ...
      ...
      } else if let num = settings[kIsAlienWordleKey] as? NSNumber {
          self.isAlienWordle = num.boolValue
      } else if let str = settings[kIsAlienWordleKey] as? String {
          // Gracefully handle string-y booleans like "true"/"false"
          switch str.lowercased() {
          case "true", "1", "yes", "on": self.isAlienWordle = true
          case "false", "0", "no", "off": self.isAlienWordle = false
          default:
              print("⚠️ Invalid string for \(kIsAlienWordleKey): \(str)")
          }
      }
    // START YOUR CODE HERE
  }
